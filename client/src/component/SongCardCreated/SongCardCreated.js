...
export default function SongCardCreated(props) {
  ...
  const getSongDetails = async () => {
    let result = await axios.get(`/api/userSongs/getOne/${props.id}`);
  ...
  const deleteSong = async (e) => {
    e.preventDefault();
    const { data } = await axios.delete(`/api/userSongs/delete/${props.id}`);
  ...
  const doUpdate = async (e) => {
    ...
    await axios
      .put(
        `http://localhost:5000/api/userSongs/update/${props.id}`,
